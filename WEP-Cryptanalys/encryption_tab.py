...
class EncryptionTab:
    ...
    def create_widgets(self):
        ...
        key_frame = ttk.LabelFrame(main_frame, text="🔐 WEP Key Configuration", 
                                   padding="15", style='Modern.TLabelframe')
        key_frame.grid(row=0, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        
        ttk.Label(key_frame, text="Key Size:", style='Modern.TLabel').grid(row=0, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Label(key_frame, text="bits", style='Modern.TLabel').grid(row=0, column=2, sticky=tk.W)
        
        ttk.Label(key_frame, text="WEP Key:", style='Modern.TLabel').grid(row=1, column=0, sticky=tk.W, pady=5)
        self.key_entry = ttk.Entry(key_frame, width=30, style='Modern.TEntry')
        self.key_entry.grid(row=1, column=1, columnspan=2, sticky=(tk.W, tk.E), padx=5, pady=5)
        self.key_entry.insert(0, "12345")
        
        ttk.Button(key_frame, text="✓ Set Key", command=self.set_key, 
                  style='Accent.TButton').grid(row=1, column=3, padx=5, pady=5)
        
        self.key_status_label = ttk.Label(key_frame, text="⚠ No key set", style='Error.TLabel')
        self.key_status_label.grid(row=2, column=0, columnspan=4, sticky=tk.W, pady=5)
        
        # Packet Generation Section
        packet_frame = ttk.LabelFrame(main_frame, text="📦 Packet Generation", 
                                      padding="15", style='Modern.TLabelframe')
        packet_frame.grid(row=1, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10), padx=(0, 5))
        
        ttk.Label(packet_frame, text="Packet Type:", style='Modern.TLabel').grid(row=0, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Label(packet_frame, text="Data Size:", style='Modern.TLabel').grid(row=1, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Label(packet_frame, text="Number of Packets:", style='Modern.TLabel').grid(row=2, column=0, sticky=tk.W, pady=5)
        ...
        weak_iv_check = ttk.Checkbutton(packet_frame, text="⚠ Generate Weak IVs (for testing)",
                                        variable=self.weak_iv_var, style='Modern.TCheckbutton')
        weak_iv_check.grid(row=3, column=0, columnspan=2, sticky=tk.W, pady=5)
        
        ttk.Label(packet_frame, text="Weak IV Type:", style='Modern.TLabel').grid(row=4, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Button(packet_frame, text="🚀 Generate & Encrypt", 
                  command=self.generate_and_encrypt,
                  style='Success.TButton').grid(row=5, column=0, columnspan=2, pady=15)
        
        # Encryption Results Section
        results_frame = ttk.LabelFrame(main_frame, text="📄 Encryption Results", 
                                       padding="15", style='Modern.TLabelframe')
        ...
        # Packet List Section
        list_frame = ttk.LabelFrame(main_frame, text="📋 Encrypted Packets", 
                                    padding="15", style='Modern.TLabelframe')
        ...
        # Packet operations
        ops_frame = ttk.Frame(list_frame, style='Modern.TFrame')
        ops_frame.grid(row=1, column=0, columnspan=2, pady=10)
        
        ttk.Button(ops_frame, text="👁 View Selected", command=self.view_selected_packet,
                  style='Modern.TButton').pack(side=tk.LEFT, padx=5)
        ttk.Button(ops_frame, text="🔓 Decrypt Selected", command=self.decrypt_selected_packet,
                  style='Modern.TButton').pack(side=tk.LEFT, padx=5)
        ttk.Button(ops_frame, text="🗑 Clear All", command=self.clear_packets,
